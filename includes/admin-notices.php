...
function jwplayer_admin_show_admin_notice() {
	...
	if ( !get_option ( 'options_jw_platform_api_key' ) || !get_option('options_jw_platform_api_secret')) {
	  if ( isset( $_GET['page'] ) && 'acf-options-podcast-plugin-settings' === sanitize_text_field( $_GET['page'] ) ) {// input var okay
	    return;
	  } else {
	    $login_url = get_admin_url( null, 'admin.php?page=acf-options-podcast-plugin-settings' );
	...
	#Show validation admin notice if credentials weren't accepted
		...
		$validation = get_option('fccpod_jw_api_authorized');
