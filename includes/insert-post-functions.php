...
function fccpod_update_title_and_slug( $post_id, $post, $update ) {
	...
	if ( 'podcasts' == $post->post_type && 'publish' == $post->post_status ) {
		...
		if ( ! $post_update ) {
			...
			fccpod_insert_segment_post( $post_id, $post, $update );
		} elseif ( $update ) {
			# If this is an update, update the segment posts
			fccpod_insert_segment_post_update( $post_id, $post, $update );
		}
	}
}
 ...
 * @version 0.16.05.24
 */
function fccpod_insert_segment_post( $post_id, $post, $update ) {
    #Get user by email
  ...
  * @version 0.16.02.21
  */
function fccpod_insert_segment_post_update( $post_id, $post, $update ) {
